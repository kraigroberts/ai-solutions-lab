...
class LLMRouter:
    ...
    async def chat(
        ...
        try:
            ...
            messages = self._prepare_messages(message, system_prompt, conversation_history)
            ...
            if backend and backend != self.settings.model_backend:
                print(f"Fallback: Trying default backend '{self.settings.model_backend}'")
