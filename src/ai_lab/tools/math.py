...
class MathTool:
            ...
            if isinstance(result, (int, float)):
                ...
                if isinstance(result, int):
                    ...
                    if result.is_integer():
                        ...
                        formatted_result = f"{result:.10g}"  # Use g format for smart precision
    ...
    async def calculate_percentage(self, input_data: str) -> str:
        ...
        try:
            ...
            match = re.match(r'increase\s+(\d+(?:\.\d+)?)\s+by\s+(\d+(?:\.\d+)?)%', input_data)
            if match:
                base = float(match.group(1))
                percentage = float(match.group(2))
                increase = (percentage / 100) * base
                result = base + increase
                return f"Increase {base} by {percentage}% = {base} + {increase} = {result}"
        ...
        try:
            ...
            if 'x' in left_side:
                ...
                if '+' in left_side:
                    ...
                    if len(terms) == 2:
                        if 'x' in terms[0]:
                            a = float(terms[0].replace('x', '')) if terms[0] != 'x' else 1.0
                            b = float(terms[1]) if terms[1] else 0.0
                        else:
                            a = float(terms[1].replace('x', '')) if terms[1] != 'x' else 1.0
