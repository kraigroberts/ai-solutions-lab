...
class SearchTool:
    ...
    async def search(self, input_data: str) -> str:
        ...
        try:
            ...
            if not self.retriever.is_loaded():
                return (
                    "Search tool is not available. No document index has been loaded."
                )
    ...
    async def get_document_info(self) -> str:
        ...
        try:
            ...
            for doc in documents:
                ...
                output.append(
                    f"Total Content Length: {doc['total_content_length']} characters"
                )
                output.append("")
    ...
    async def get_index_stats(self) -> str:
        ...
        try:
            ...
            if build_info:
                ...
                output.append(
                    f"  Chunk overlap: {build_info.get('chunk_overlap', 'N/A')}"
                )
                output.append(
                    f"  Build timestamp: {build_info.get('build_timestamp', 'N/A')}"
                )
    ...
    async def search_similar(self, chunk_id: str, top_k: int = 5) -> str:
        """
