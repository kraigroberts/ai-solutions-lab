...
class Settings(BaseSettings):
    ...
    model_backend: Literal["local", "openai", "anthropic"] = Field(
        default="local",
        description="LLM provider to use for chat and generation"
    )
    
    # Embeddings Backend Configuration  
    embeddings_backend: Literal["local", "openai"] = Field(
        default="local",
        description="Embeddings provider for vector operations"
    )
    ...
    pinecone_api_key: Optional[str] = Field(
        default=None,
        description="Pinecone API key for cloud vector store"
    )
    ...
    local_model_max_tokens: int = Field(
        default=512,
        description="Maximum tokens to generate with local models"
    )
    ...
    models_dir: Path = Field(
        default=Path("./models"),
        description="Directory for local model files"
    )
    
    # Server Configuration
    host: str = Field(
        default="0.0.0.0",
        description="Host to bind the FastAPI server to"
    )
    ...
    rate_limit_per_minute: int = Field(
        default=60,
        description="Rate limit for API endpoints per minute"
    )
