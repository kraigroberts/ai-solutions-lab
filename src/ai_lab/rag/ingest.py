...
class PDFLoader(DocumentLoader):
    ...
    async def load(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError("pypdf is required for PDF loading. Install with: pip install pypdf")
        except Exception as e:
...
class DocumentIngester:
    ...
    async def build_index(
        ...
        index_size_mb = (index_file.stat().st_size + metadata_file.stat().st_size) / (1024 * 1024)
    ...
    async def list_indexes(self, index_dir: Optional[Path] = None) -> List[Dict[str, Any]]:
