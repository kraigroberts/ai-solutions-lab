...
class VectorRetriever:
    ...
    def _initialize_embedding_model(self) -> None:
        ...
        try:
            if self.settings.embeddings_backend == "local":
                ...
                print(f"Local embedding model loaded for retrieval: {self.settings.embedding_model}")
            else:
    ...
    async def retrieve_by_document(
        ...
        document_chunks = [chunk for chunk in self.chunks if chunk.document_id == document_id]
        ...
        try:
            ...
            scores, indices = temp_index.search(query_vector, min(top_k, len(valid_chunks)))
