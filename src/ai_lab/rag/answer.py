...
class RAGAnswerer:
    ...
    async def _generate_answer(self, query: str, context: str, strategy: str) -> str:
...
User question: {query}"""
        ...
        try:
            response = await self.llm_router.chat(
                message=query, system_prompt=system_prompt
            )
...
User question: {query}"""
    ...
    async def answer_with_followup(
        self, query: str, followup_questions: Optional[List[str]] = None, **kwargs
    ) -> Dict[str, Any]:
        """
    ...
    async def batch_answer(self, queries: List[str], **kwargs) -> List[Dict[str, Any]]:
        """
